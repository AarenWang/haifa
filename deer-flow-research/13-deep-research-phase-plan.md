...
本文将 `12-deep-research-gap-plan.md` 中规划的深度研究能力拆分为可迭代交付的 Phase plan。
...
| --- | --- | --- | --- |
| Phase 0 | Baseline 固化 | 明确当前能力、开关、日志和测试基线 | 避免后续改动破坏现有 chat / web_search / web_fetch |
...
| Phase 3 | Source / Evidence / Citation Core | 来源注册、证据存储、引用管理、网页清洗 | 研究结论开始可追溯，重复来源被去重 |
| Phase 4 | Research Plan / Progress / Quality Gate | 研究计划、Todo 状态、质量门禁 | 深度研究不再“搜一次就总结”，而是按维度推进 |
| Phase 5 | Report / Artifact Delivery | Markdown 报告、artifact API、预览和下载 | 用户拿到可交付报告，而不是超长聊天文本 |
| Phase 6 | Context Compression / Tool Budget | 长上下文摘要、证据压缩、工具输出预算 | 支撑更多来源、更长任务，降低上下文溢出风险 |
| Phase 7 | Thread Continuity / Research Memory | thread 级来源、计划、artifact 复用 | 用户能继续扩展上一份研究，不必重复收集资料 |
| Phase 8 | Subagent 并行研究 | 子任务运行时、并发限制、结果汇总 | 复杂主题可按维度并行研究，由主 agent 统一综合 |

## 3. Phase 0：Baseline 固化
...
## 5. Phase 1：Research Mode API 与事件语义
...
## 6. Phase 2：模型驱动 Tool Calling Loop
...
## 7. Phase 3：Source / Evidence / Citation Core
...
## 8. Phase 4：Research Plan / Progress / Quality Gate

### 目标

把 deep-research skill 的方法论变成运行时约束：先计划、按维度推进、再做质量检查。
...
## 9. Phase 5：Report / Artifact Delivery

### 目标

让深度研究输出成为可交付、可预览、可下载、可追踪的 artifact。
...
## 10. Phase 6：Context Compression / Tool Budget

### 目标

支撑更多来源和更长研究任务，避免上下文溢出和网页噪声污染模型。
...
- 实现 `SourceAwareSummary`：
  - 摘要必须保留 `sourceId` / URL / citation anchor。
- 实现 `SkillRescue`：
  - 最近加载或关键 skill 内容不被错误压缩。
...
- 压缩不会丢失 sourceId、URL、关键 claim。

## 11. Phase 7：Thread Continuity / Research Memory

### 目标

让用户可以在同一 thread 中继续扩展上一份研究、补充章节或复用已有来源生成新产物。

### Backend

- Thread-level research memory：
...
- Follow-up handling：
  ...
  - “用上次来源生成 PPT / 摘要 / 表格”复用 artifact 和 evidence。
- Auto title：
...
- 用户要求补充某个章节时，不重跑全部维度。
- artifact list 能跨 run 查询。

### 验收标准

- 用户能基于已有 evidence 继续工作。
- 不需要重复搜索已读取来源。
- Follow-up 结果能引用上一轮来源并生成新的 artifact 或答案。

## 12. Phase 8：Subagent 并行研究

### 目标

在 source / evidence / report / quality 基础设施稳定后，引入并行研究能力，让复杂主题可以按维度拆分并发调查。
...
建议把 Phase 0.5 到 Phase 5 作为 Java DeerFlow Deep Research MVP。
...
Phase 0 Baseline
  ...
  -> Phase 2 AgentLoop
  -> Phase 3 Source / Evidence / Citation
  -> Phase 4 Plan / Progress / QualityGate
  -> Phase 5 Report / Artifact
  -> Phase 6 Compression / Tool Budget
  -> Phase 7 Thread Memory
  -> Phase 8 Subagent Parallelism
```

关键依赖说明：

- Phase 1 依赖 Phase 0.5 的 canonical `threadId`，否则 research options、事件、artifact 后续缺少稳定归属。
- Phase 2 依赖 Phase 1 的 research mode 和事件边界。
- Phase 3 依赖 Phase 2 的 fetch 结果进入标准 tool loop。
- Phase 4 依赖 Phase 3 的 evidence / citation，否则 quality gate 无法可靠评分。
- Phase 5 依赖 Phase 3 和 Phase 4，否则报告缺少可追溯证据和质量检查。
- Phase 6 可以在 Phase 5 后开始，也可以在 Phase 3 后提前做 tool output budget 的轻量版本。
- Phase 8 必须放在 source / evidence / quality / report 稳定后，否则并行只会放大无结构文本问题。
...
| 风险 | 影响 | 控制方式 |
| --- | --- | --- |
| Thread 过晚引入 | Source、Evidence、Artifact 归属反复返工 | Phase 0.5 先做轻量 Thread 模型，Phase 7 再做高级 memory |
| Agent loop 无限循环 | 成本和延迟失控 | max steps、max tool calls、timeout、stop reason |
...
| 过早引入 subagent | 并行产生无结构文本 | Phase 8 后置，依赖 evidence/report/quality 基础设施 |
| Artifact 路径安全 | 文件泄露或路径穿越 | ArtifactService 只暴露登记文件，下载接口做路径校验 |
| 前端事件膨胀 | 用户看不懂进度 | 分层展示：摘要进度默认展示，详细 trace 可展开 |

## 17. 完成后的能力边界

完成 Phase 1 到 Phase 5 后，Java DeerFlow 应从“搜索增强聊天”升级为“最小可用深度研究 agent”。

完成 Phase 6 到 Phase 8 后，Java DeerFlow 应具备更接近 DeerFlow Python 的长任务研究能力：
